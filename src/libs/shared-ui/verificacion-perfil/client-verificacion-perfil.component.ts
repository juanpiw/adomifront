...
} from '../../../app/services/client-verification.service';

@Component({
  selector: 'app-client-verificacion-perfil',
  standalone: true,
...
export class ClientVerificacionPerfilComponent implements OnInit {
  @Output() statusChange = new EventEmitter<ClientVerificationStatus>();
  ...
  verificationRecord: ClientVerificationRecord | null = null;
  rejectionReason: string | null = null;
  documentNumber: string = '';
  documentType: 'cedula' | 'pasaporte' | 'licencia' = 'cedula';
  verificationId: number | null = null;

  private verificationService = inject(ClientVerificationService);
    ...
    this.verificationService.getStatus()
      ...
      .subscribe({
        next: (response) => {
          ...
          const profileStatus = response?.profile?.verification_status as ClientVerificationStatus | undefined;
          const recordStatus = (this.verificationRecord?.status as ClientVerificationStatus) || 'none';
          this.verificationStatus = profileStatus || recordStatus || 'none';
      this.statusChange.emit(this.verificationStatus);
        ...
        error: (error) => {
          console.error('[CLIENT_VERIFICACION] Error cargando estado', error);
  ...
  async uploadDocuments() {
    ...
    try {
      ...
      console.error('[CLIENT_VERIFICACION] Error al subir documentos:', error);
